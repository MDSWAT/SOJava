    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div>
        <h1 className="text-2xl font-bold tracking-tight dark:text-white">
          Sistem Settings & module loader
        </h1>
        <p className="text-xs text-slate-500 dark:text-slate-400">
          Activarea modulelor, auditul configurației active de LDAP și acordarea granulară a permisiunilor prin matricea de control (RBAC).
        </p>
      </div>
      ...
      {error ? (
        <div className="p-8 text-center glass-panel rounded-2xl space-y-2">
          ...
          <h3 className="font-bold text-white text-md">Setări Indisponibile</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            <div className="glass-panel p-5 rounded-2xl space-y-4">
              ...
              <div className="space-y-3.5 pt-2">
                {Object.entries(modules).map(([modName, isActive]) => (
                  <div key={modName} className="flex justify-between items-center bg-slate-900/40 p-2.5 rounded-xl border border-slate-850">
                    <div className="min-w-0 pr-2">
                      <p className="text-xs font-bold text-slate-200 uppercase truncate">
            ...
            <div className="glass-panel p-5 rounded-2xl space-y-3">
              ...
              <div className="text-[10px] space-y-2 leading-relaxed text-slate-400 font-semibold">
                <div className="flex justify-between items-center py-1.5 border-b border-slate-800/40">
                  <span>Server LDAP:</span>
                  <span className="font-mono text-white">ldap://ccl-dc.sidesi.local</span>
                </div>
                <div className="flex justify-between items-center py-1.5 border-b border-slate-800/40">
                  <span>Port protocol:</span>
                  <span className="font-mono text-white">389 (SSL: 636)</span>
                </div>
                <div className="flex justify-between items-center py-1.5 border-b border-slate-800/40">
                  <span>Baza sAMAccount:</span>
                  <span className="font-mono text-white">OU=Users,DC=sidesi,DC=local</span>
                </div>
                <div className="flex justify-between items-center py-1.5">
                  <span>Celery Auto-Sync interval:</span>
                  <span className="font-mono text-emerald-400">Zilnic (Daily @ 02:00)</span>
                </div>
              </div>
            </div>
          </div>
              ...
              {isLoading ? (
                ...
                <div className="overflow-x-auto border border-slate-850 rounded-xl">
                  <table className="w-full text-left border-collapse text-xs font-semibold">
                    <thead>
                      <tr className="bg-slate-900 border-b border-slate-850 text-slate-400 font-bold h-10 select-none">
                    ...
                    <tbody className="divide-y divide-slate-850 font-medium">
                      {permissions.map((perm) => (
                        <tr key={perm.id} className="hover:bg-slate-900/10 text-slate-300 h-11">
                          <td className="px-4 py-1.5">
                            <p className="font-bold text-slate-200 font-mono text-[10px]">{perm.code}</p>
                              ...
                              <td key={role.id} className="px-3 py-1.5 text-center">
                                <label className="inline-flex items-center justify-center cursor-pointer select-none">
                                  <input
                                    ...
                                    className="rounded border-slate-850 text-white accent-sidesi-500 w-4 h-4 cursor-pointer disabled:opacity-50"
                                  />
                                </label>
                              </td>
                            );
